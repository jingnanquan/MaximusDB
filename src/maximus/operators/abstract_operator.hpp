...
public:
    ...
    void add_input(DeviceTablePtr input, int port = 0) {
        ...
        // the input passed must be of a batch type (e.g. TableBatch, arrow::RecordBatch, arrow::compute::ExecBatch and similar)
        assert(input.is_batch_type());
    ...
    TablePtr export_table() {
        ...
        if (has_more_batches(true)) {
            ...
            table = merge_batches(ctx_, outputs_, output_schema);
