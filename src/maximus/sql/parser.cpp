...
Status Parser::qp_order_by(std::vector<hsql::OrderDescription*>* order_by,
    ...
    auto order_by_plan = std::make_shared<QueryNode>(
        EngineType::ACERO, NodeType::ORDER_BY, std::move(properties), ctx);
...
Status Parser::qp_group_by(const hsql::GroupByDescription* group_by,
    ...
    auto group_by_plan = std::make_shared<QueryNode>(
        EngineType::ACERO, NodeType::GROUP_BY, std::move(properties), ctx_);
...
Status Parser::qp_from_join(
    ...
    std::shared_ptr<QueryNode> query_plan = std::make_shared<QueryNode>(
        EngineType::ACERO, NodeType::HASH_JOIN, std::move(join_config), ctx_);
    assert(query_plan);
...
Status Parser::qp_from_table_ref(
    ...
    switch (table->type) {
        case hsql::TableRefType::kTableName: {
            assert(table->name);
            std::string table_path = db_catalogue_->table_path(table->name);
            ...
            query_plan = std::make_shared<QueryNode>(
                EngineType::NATIVE, NodeType::TABLE_SOURCE, std::move(properties), ctx_);
            assert(query_plan);
    ...
    switch (statement->type()) {
        case hsql::kStmtSelect: {
            ...
            auto sink            = std::make_shared<QueryNode>(
                EngineType::NATIVE, NodeType::TABLE_SINK, sink_properties, ctx_);
